 ...
 *
 * @param {number} count - Number of articles in this batch
 * @param {string} [topic="ai"] - "ai" or "tech"
 * @param {number} [startIndex=1] - 1-based index of the first article in this batch
 * @returns {string}
 */
function buildSystemPrompt(count, topic = "ai", startIndex = 1) {
  const toBengali = (n) =>
    String(n).replace(/\d/g, (d) => "০১২৩৪৫৬৭৮৯"[d]);
  ...
  return (
    ...
    `<b>${cfg.header} — ${toBengali(startIndex)}–${toBengali(endIndex)} নম্বর খবর</b>\n\n` +
    ...
    `- [নম্বর] এর জায়গায় আর্টিকেলের ক্রমিক সংখ্যা দাও — প্রথম আর্টিকেলের নম্বর ${startIndex} দিয়ে শুরু করো (যেমন: ${startIndex}, ${startIndex + 1}, ${startIndex + 2}…) — ব্যবহারকারী এই নম্বর পাঠিয়ে বিস্তারিত জানতে পারবেন\n` +
 ...
 * @returns {string}
 */
function buildUserMessage(articles, startIndex = 1) {
 ...
 * @param {number} [startIndex=1] - 1-based index of the first article (for multi-batch runs)
 * @returns {Promise<{ summary: string, usage: { input_tokens: number, output_tokens: number, total_tokens: number } }>}
 */
async function summarizeArticles(articles, topic = "ai", startIndex = 1) {
  const systemPrompt = buildSystemPrompt(articles.length, topic, startIndex);
  const userMessage = buildUserMessage(articles, startIndex);
...
/**
 * Summarise articles in batches of `batchSize`, calling Claude once per batch.
 * Returns all summaries and the combined token usage across all batches.
 *
 * @param {Array} articles
 * @param {string} [topic="ai"]
 * @param {number} [batchSize=25]
 * @returns {Promise<{ summaries: string[], usage: { input_tokens: number, output_tokens: number, total_tokens: number } }>}
 */
async function summarizeBatched(articles, topic = "ai", batchSize = BATCH_SIZE) {
  const chunks = [];
  ...
  const summaries = [];
  const combinedUsage = { input_tokens: 0, output_tokens: 0, total_tokens: 0 };

  for (let i = 0; i < chunks.length; i++) {
    const startIndex = i * batchSize + 1;
    const chunk = chunks[i];
    const { summary, usage } = await summarizeArticles(chunk, topic, startIndex);
    summaries.push(summary);
    combinedUsage.input_tokens += usage.input_tokens;
    combinedUsage.output_tokens += usage.output_tokens;
    combinedUsage.total_tokens += usage.total_tokens;
  }

  console.log(
    `[summarize] Batched run complete — ${chunks.length} batch(es), ` +
      `total tokens: ${combinedUsage.total_tokens}`
  );

  return { summaries, usage: combinedUsage };
}

module.exports = { summarizeArticles, summarizeBatched };

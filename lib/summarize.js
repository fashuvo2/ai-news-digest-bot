...
function buildSystemPrompt(count) {
  ...
  return (
    `তুমি একজন AI সংবাদ বিশ্লেষক। নিচের AI জগতের সাম্প্রতিক ${toBengali(count)}টি খবর পড়ে বাংলায় একটি সংক্ষিপ্ত ও তথ্যবহুল ডাইজেস্ট তৈরি করো। কোনো Markdown চিহ্ন যেমন #, ##, *, ** ব্যবহার করবে না।\n\n` +
    `ডাইজেস্টের শুরুতে হুবহু এই লাইনটি লেখো (HTML ট্যাগসহ):\n` +
    `<b>🤖 AI সংবাদ ডাইজেস্ট — ${toBengali(count)}টি খবর</b>\n\n` +
    `প্রতিটি খবরের জন্য নিচের ফরম্যাট অনুসরণ করো:\n\n` +
    `——————————————\n` +
    `<b>[প্রাসঙ্গিক ইমোজি] [মূল ইংরেজি শিরোনাম]</b>\n` +
...
async function summarizeArticles(articles) {
  const systemPrompt = buildSystemPrompt(articles.length);
